...
data <- read_csv(here("data/amr_data.csv"))

# Info box values ---------------------------------------------------------

num_sightings <- unique(table(data$Especes))
unique_genius <- unique()unique_genius <- length(unique(data$Genre))
unique_locations <- length(unique(data$SITE))
